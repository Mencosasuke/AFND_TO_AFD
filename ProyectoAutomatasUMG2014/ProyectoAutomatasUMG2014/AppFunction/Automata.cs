...
namespace ProyectoAutomatasUMG.AppFunction
{
    class Automata
    {
        ...
        public List<String> CerraduraEpsilon(List<String> coleccionEstados, List<Transicion> listaFuncTransicionAutomataND, int cont, int index)
        {
            ...
            {
                foreach (Transicion trans in listaFuncTransicionAutomataND.Where(f => f.estado.Equals(coleccionEstados[i]) && f.simbolo.Equals("e") && !coleccionEstados.Contains(f.proximoEstado)))
                {
